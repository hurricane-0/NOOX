...
AppWiFiManager::AppWiFiManager(LLMManager& llm, ConfigManager& config) 
    : llmManager(llm), configManager(config) {
}
...
bool AppWiFiManager::connectToWiFi(const String& ssid) {
    ...
    if (password.length() == 0) {
        Serial.printf("SSID %s not found in configuration.\n", ssid.c_str());
