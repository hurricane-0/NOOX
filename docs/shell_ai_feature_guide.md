...
ESP32-S3 将被配置为一个 USB 复合设备，同时提供以下两个关键接口：

*   **`HID` (Human Interface Device):** 保留现有的模拟键盘/鼠标功能。
*   **`CDC` (Communication Device Class):** 创建一个虚拟串口，作为 ESP32 与主机代理程序之间双向数据通信的桥梁。

此外，ESP32-S3 还运行一个 **Web 服务器**（通过 WiFi 连接），用于提供主机代理程序的可执行文件下载服务。

### 2.2 主机代理程序

这是一个运行在用户计算机上的 **自包含原生可执行文件** (无需安装任何解释器或依赖)。其核心职责包括：

*   **自动下载与启动：** ESP32 通过 `HID` 模拟键盘操作，在主机上打开 PowerShell 窗口，使用 `Invoke-WebRequest` 从 ESP32 的 Web 服务器下载代理程序到临时目录，然后自动运行。随后，自动检测并连接到 ESP32 提供的 `CDC` 虚拟串口。
...
*   **通信测试：** 启动后自动发送 `linktest` 消息，测试与设备的双向通信是否正常。
...
*   **文件系统与 Web 服务器管理:**
    *   配置并挂载 LittleFS 文件系统，用于存储 Web 界面文件和主机代理程序。
    *   将预编译好的主机代理可执行文件存放在 `data/agent/` 目录下。
    *   配置 Web 服务器，提供 `/api/agent/download` 端点，支持按平台（windows、macos、linux）下载对应的代理程序。

### 3.2 主机代理程序开发

*   **语言选择与跨平台编译:**
    *   使用 **Go** 开发，因为它能轻松生成无依赖的单一可执行文件。
...
2.  **自动识别:** 计算机自动识别出两个设备：一个标准的键盘/鼠标，一个虚拟串口。
3.  **WiFi 连接:** ESP32 需要先连接到 WiFi 网络（可通过 Web 界面或 HID 键盘操作配置）。
4.  **启动代理:** ESP32 通过 `HID` 模拟键盘操作，在主机上打开 PowerShell 窗口，使用 `Invoke-WebRequest` 从 ESP32 的 Web 服务器（`http://<设备IP>/api/agent/download?platform=windows`）下载代理程序到临时目录，然后自动运行。
5.  **通信测试:**
    *   Go 代理程序启动后，首先发送 `linktest` 消息测试与设备的双向通信，确保连接正常。
6.  **开始交互:** 代理程序完成初始化后，用户即可在当前终端窗口中输入自然语言与 AI 对话。所有交互（包括 Shell 命令的执行和 AI 的回复）都会在这个窗口中无缝进行。
7.  **命令执行:** 
